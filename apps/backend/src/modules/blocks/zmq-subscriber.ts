// Bitcoin Knots ZMQ block notifiers are silent until the node finishes Initial Block Download
...
async function startBlockSubscriber(): Promise<void> {
	...
	subscriber.connect(`tcp://${process.env['BITCOIND_IP'] || '127.0.0.1'}:${process.env['ZMQ_HASHBLOCK_PORT'] || '28334'}`)
